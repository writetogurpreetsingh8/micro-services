...
public class ConcurrenyConversionController {
	...
	private ResponseEntity<ConcurrenyConversionBean> convertConcurrenyMessaging(@PathVariable("from") String from,@PathVariable("to") String to,@PathVariable("quantity") BigDecimal quantity) throws JMSException {
		...
		LOGGER.info("data send to currency-exchange-services........................");
		Message message = 	(TextMessage)jmsTemplate.receive(currencyConversionQueue);
		LOGGER.info("data received from currency-exchange-services........................");
		...
		*/
		Object response = message.getBody(Object.class);
		...
		ConcurrenyConversionBean newBean = new ConcurrenyConversionBean(1L,from,to,
				new BigDecimal(response.toString()),quantity,quantity.multiply(new BigDecimal(response.toString())),0000);
